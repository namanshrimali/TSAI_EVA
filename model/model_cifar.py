...
class ModelCifar(nn.Module):
    def __init__(self):
        ...
        self.conv_lyr1 = nn.Sequential(
            nn.Conv2d(3, 32, 3, padding= 1, padding_mode= 'replicate'), #3, 32
            nn.ReLU(),
            nn.BatchNorm2d(32),
            nn.Dropout(0.15),

            nn.Conv2d(32, 64, 3, padding= 1 , padding_mode= 'replicate'), # 5, 32
            nn.ReLU(),
            nn.BatchNorm2d(64),
            nn.Dropout(0.15),
        ...
        self.dpthwse_sprbl_blk1 = nn.Sequential(
            nn.Conv2d(64, 64, 3, padding= 1, padding_mode= 'replicate', groups= 64), #12, 16
            nn.ReLU(),
            nn.BatchNorm2d(64),
            nn.Dropout(0.15),
        ...
        self.dpthwse_sprbl_blk2 = nn.Sequential(
    ...
    def forward(self, x):
        x = self.conv_lyr1(x)
        x = self.dpthwse_sprbl_blk1(x)
        x = self.dpthwse_sprbl_blk2(x)
        x = self.dil_lyr1(x)
        x = self.fnl_lyr(x)
        x = x.view(-1, 10)
